...
class Character():
    def __init__(self, x, y, health, mob_animations, char_type, boss, size, enemy = True):
    ...
    def move(self, dx, dy, obstacle_tiles, exit_tile = None):
        ...
        # checking direction to face
        if dx < 0:
            self.flip = True
        if dx > 0:
            self.flip = False

        # regulate diagonal speed
        if dx != 0 and dy != 0:
            dx = dx * (math.sqrt(2) / 2)
            dy = dy * (math.sqrt(2) / 2)
        ...
        # camera scroll is only applicable to player
        if self.char_type == 0:
            # check collision with exit ladder
            if exit_tile[1].colliderect(self.rect):
                # check if player is in the center of the exit tile
    ...
    def ai(self, player, obstacle_tiles, screen_scroll, fireball_image):
        ...
        # check if line_of_sight passes through an obstacle
        for obstacle in obstacle_tiles:
        ...
        if not clipped_line and dist > constants.RANGE and self.enemy:
        ...
        if self.alive:
            if not self.stunned:
                ...
                # attack player
                if dist < constants.ATTACK_RANGE and player.hit == False and self.enemy:
                    player.health -= 10
                    player.hit = True
    ...
    def update(self):
        ...
        # timer to reset player taking damage
        hit_cooldown = 1000
        if self.char_type == 0:
            if self.hit and ((pygame.time.get_ticks() - self.last_hit) > hit_cooldown):
                self.hit = False

        # check action of player (idle or running)
        if self.running:
    ...
    def draw(self, surface):
        ...
        if self.char_type == 0 or self.char_type == 4 or self.char_type == 5 or self.char_type == 2:
            surface.blit(flipped_image, (self.rect.x, self.rect.y - constants.SCALE * constants.OFFSET + 4))
        else:
